...
// ==========================================
function getDashboardData(dataMentah, bulanFilter, mesinFilter, tanggalFilter = "all") {
    let mapBulan = new Set(), mapMesin = new Set(), mapTanggal = new Set();
    
    dataMentah.forEach(item => {
        if (item.bulan) mapBulan.add(item.bulan);
        if (item.mesin) mapMesin.add(item.mesin);
        // Kumpulkan list tanggal dari sheet data mentah
        if (item.details) {
            item.details.forEach(det => {
                if (det.tanggal) mapTanggal.add(det.tanggal);
            });
        }
    });
    ...
    // Sort tanggal berdasarkan urutan kemunculan numerik/data asli
    const listTanggal = Array.from(mapTanggal);
    ...
    dataMentah.forEach(item => {
        ...
        if (matchBulan && matchMesin) {
            // Salin item agar tidak merusak data cache asli saat details di-filter
            let clonedItem = JSON.parse(JSON.stringify(item));
            
            // Lakukan pemfilteran horizontal berdasarkan tanggal jika tidak bernilai "all"
            if (tanggalFilter !== "all") {
                clonedItem.details = clonedItem.details.filter(det => det.tanggal === tanggalFilter);
            }

            dataFiltered.push(clonedItem);
            if (clonedItem.details.length > maxOutputs) {
                maxOutputs = clonedItem.details.length;
            }
        }
    });

    return { bulanFilter, mesinFilter, tanggalFilter, listBulan, listMesin, listTanggal, data: dataFiltered, maxOutputs };
}
...
function renderDropdowns(payload) {
    ...
    if (btnSuccess && currentFilename === "index.html") {
        btnSuccess.href = `pencapaian.html?bulan=${payload.bulanFilter}&mesin=${payload.mesinFilter}&tanggal=${payload.tanggalFilter}`;
    }
    
    const btnSecondary = document.querySelector('.btn-secondary');
    if (btnSecondary && currentFilename === "index.html") {
        btnSecondary.href = `diagram.html?bulan=${payload.bulanFilter}&mesin=${payload.mesinFilter}&tanggal=${payload.tanggalFilter}`;
    }
}
...
function renderDashboardTable(payload) {
    ...
    if (payload.data.length > 0) {
        ...
        // Buat hitungan colspan dinamis per blok tanggal yang ter-render
        let hitungShiftPerTanggal = {};
        firstRowDetails.forEach(detail => {
            if (detail.tanggal) {
                hitungShiftPerTanggal[detail.tanggal] = (hitungShiftPerTanggal[detail.tanggal] || 0) + 1;
            }
        });
        ...
        firstRowDetails.forEach(detail => {
            if (!setHeaderTgl.has(detail.tanggal)) {
                ...
                let colSpanValue = hitungShiftPerTanggal[detail.tanggal] || 3;
                headerRow1 += `<th colspan="${colSpanValue}" style="text-align:center; font-weight: bold; border-bottom: 1px solid #cbd5e1;">📅 ${detail.tanggal || 'N/A'}</th>`;
            }
        });
        
        // Render penamaan shift pembagi kolom bawah secara adaptif
        firstRowDetails.forEach((detail, idx) => {
            headerRow2 += `<th style="text-align:center; min-width:110px;">Shift ${detail.shift || (idx % 3) + 1}</th>`;
        });
    } else {
...
async function initApp() {
    ...
    const filterTanggal = urlParams.get('tanggal') || 'all'; // PENAMBAHAN: Tangkap nilai parameter tanggal
    ...
    if (filterForm) {
        ...
        filterForm.onsubmit = async (e) => {
            ...
            const targetTanggal = formData.get('tanggal') || 'all'; // PENAMBAHAN: Ambil data tanggal dari form select
            ...
            targetUrl.searchParams.set('tanggal', targetTanggal); // PENAMBAHAN: Tulis state tanggal ke URL
            ...
            const payload = getDashboardData(dataBulanForm, targetBulan, targetMesin, targetTanggal);
    ...
    const payload = getDashboardData(dataMentah, filterBulan, filterMesin, filterTanggal);
    renderDropdowns(payload);
